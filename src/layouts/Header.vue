...
<template>
    <q-header v-model="defStore().config.isReveal" reveal :bordered="false" style="color: inherit; background: inherit">
        <q-toolbar>
            <q-btn
                v-if="['all', 'left'].includes(defStore().config.menuPosition) && defStore().config.showMenu"
                @click="defStore().$patch((state: any) => (state.config.drawerMenu = !state.config.drawerMenu))"
                round
                dense
                icon="menu"
            />
       ...
       -->
            ...
            <div
                ...
                "
            >
                <Menu :routes="$router.options.routes" :selMenu="defStore().selMenu" mode="horizontal"></Menu>
            </div>
       ...
       -->
            ...
            <q-btn
                ...
                glossy
                icon="more_vert"
                @click="defStore().$patch((state: any) => (state.config.drawerSetting = !state.config.drawerSetting))"
            />
        </q-toolbar>
